...
(check-equal? 
 g
 (graph (t <-6-- s)  (y <-7-- s)  (x <-5-- t)  (y <-8-- t) (z <--4-- t)
        (t <--2-- x) (x <--3-- y) (z <-9-- y)  (s <-2-- z) (x <-7-- z)))
...
(check-equal? 
 g2
 (graph (t <-3-- s) (y <-5-- s) (x <-6-- t) (y <-2-- t) (z <-2-- x)
        (t <-1-- y) (x <-4-- y) (z <-6-- y) (s <-3-- z) (x <-7-- z)))
...
(define g3 (graph (s --3-> a) (s --5-> c) (s --2-> e)
                  (a ---4-> b)
                  (b --4-> g)
                  (c --6-> d)
                  (d ---3-> c) (d --8-> g)
...
(check-equal?
 dag
 (graph (s <-5-- r) (t <-3-- r) (t <-2-- s)  (x <-6-- s) (x <-7-- t)
        (y <-4-- t) (z <-2-- t) (y <--1-- x) (z <-1-- x) (z <--2-- y)))
...
;;;;;;;;;; dijkstra tests ;;;;;;;;;;
(define g-dij (graph (s --10-> t) (s --5-> y)
                     (t --1-> x)  (t --2-> y)
                     (x --4-> z)
                     (y --3-> t)  (y --9-> x) (y --2-> z)
                     (z --6-> x)  (z --7-> s)))
